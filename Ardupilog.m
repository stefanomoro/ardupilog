...
%                         keyboard
        ...
        % truth is it was LOGGED at this time, not received)
            ...
            if isprop(obj.GPS(1), 'TimeUS') % This is a typical Ardupilot logs
                timestr = 'TimeUS';
                timeconvert = 1;
            elseif isprop(obj.GPS(1), 'TimeMS') || isprop(obj.GPS(1), 'T') % This is one of the old Solo logs
                timestr = 'T';
                timeconvert = 1e3;
%             elseif isprop
            else
            ...
            if isprop(obj.GPS(1), 'GWk') % (typical)
                wkstr = 'GWk';
            elseif isprop(obj.GPS(1), 'Week') % (Old Solo log)
                wkstr = 'Week';
            else
                error('Unsupported week-type in obj.GPS')
            end

            if isprop(obj.GPS(1), 'GMS') % (typical)
                gpssecstr = 'GMS';
            elseif isprop(obj.GPS(1), 'T') || isprop(obj.GPS(1), 'TimeMS') % (Old Solo log)
                gpssecstr = 'TimeMS';
            else
                error('Unsupported GPS-seconds-type in obj.GPS')
            end

            if ~isempty(obj.GPS(1).(timestr))
                % Get the first valid time data from the log
                first_ndx = find(obj.GPS(1).(wkstr) > 0, 1, 'first');
            else
                first_ndx = [];
            end
            if ~isempty(first_ndx)
            
                temp = obj.GPS(1).(timestr);
                recv_timeUS = temp(first_ndx)*timeconvert;
                temp = obj.GPS(1).(wkstr);
                recv_GWk = temp(first_ndx);
                temp = obj.GPS(1).(gpssecstr);
